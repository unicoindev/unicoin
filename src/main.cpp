...
// Copyright (c) 2013 UniCoin developers
...
const string strMessageMagic = "UniCoin Signed Message:\n";
...
static const int64 nStartSubsidy = 8 * COIN;
static const int64 nMinSubsidy = 1 * COIN;
static const int64 nfirstblocks = 0.2014 * COIN;
...
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 1/4 blocks

static const int64 nAveragingInterval = nInterval * 10; // 40 blocks
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; // 1 hour 20 minutes

static const int64 nMaxAdjustDown = 20; // 20% adjustment down
static const int64 nMaxAdjustUp = 20; // 20% adjustment up
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UniCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UniCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running UniCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UniCoinMiner terminated\n");
        throw;
    }
}
